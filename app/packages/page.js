...
export default function PackagesPage() {
    ...
    return (
        <div ref={containerRef} className="bg-brand-white min-h-screen">
            ...
            {/* Page Hero Banner */}
            <section className="relative h-[420px] pt-32 flex flex-col justify-end bg-brand-charcoal overflow-hidden group">
                <div className="absolute inset-0 opacity-40">
                    <img src="https://images.unsplash.com/photo-1488646953014-85cb44e25828?q=80&w=2000" alt="Collections Hero" className="w-full h-full object-cover transition-transform duration-10000 group-hover:scale-110" />
                ...
                <div className="container mx-auto px-6 md:px-12 relative z-10 pb-16">
                    ...
                    <p className="reveal text-white/60 text-lg max-w-2xl leading-relaxed">
                        From the peaks of the Himalayas to the sunsets of Santorini, discover journeys precisely tailored to your soul.
                    </p>
                </div>
            </section>
                ...
                <div className="container mx-auto px-6 md:px-12 py-6 flex flex-col lg:flex-row justify-between items-center gap-8">
                    ...
                    <div className="reveal flex items-center gap-8 w-full lg:w-auto justify-between lg:justify-end">
                        <div className="flex items-center gap-4">
                            <label className="text-[10px] font-bold uppercase tracking-widest text-brand-charcoal/40">Sort By</label>
                            <select
                                value={sortBy}
                        ...
                        <div className="flex items-center gap-2 border border-brand-charcoal/10 p-1">
                            ...
                            >
                                <div className="flex flex-col gap-[2px] w-3 h-3">
                                    <div className="bg-current h-[2px] w-full"></div>
                                    <div className="bg-current h-[2px] w-full"></div>
                                    <div className="bg-current h-[2px] w-full"></div>
                                </div>
                            </button>
                        ...
                        return (
                            ...
                            >
                                <Link href={`/packages/${pkg.slug}`}>
                                    <div className={`relative overflow-hidden bg-brand-charcoal ${view === "grid" ? (isFeatured ? "aspect-16/7" : "aspect-4/5") : "flex flex-col md:flex-row h-auto md:h-64"}`}>
                                        <img
                                            src={pkg.image}
                                            alt={pkg.title}
                                        ...
                                        {view === "list" && (
                                            <div className="flex-1 p-10 flex flex-col justify-center bg-white border border-brand-charcoal/5 border-l-0">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div>
                                                        <span className="text-brand-accent text-[8px] font-bold uppercase tracking-[0.3em] mb-1 block">Rating <span className="text-brand-forest opacity-50">★ {pkg.rating}</span></span>
                                                        <h3 className="text-2xl md:text-3xl font-heading font-bold text-brand-forest">{pkg.title}</h3>
                                                    </div>
                                                    <div className="text-right">
                                                        <span className="text-sm text-brand-charcoal/40 block font-bold uppercase tracking-tighter">From</span>
                                                        <span className="text-3xl font-heading font-bold text-brand-forest">${pkg.price}</span>
                                                    </div>
                                                </div>
                                    ...
                                    {view === "grid" && (
                                        <div className="mt-6 flex justify-between items-start gap-4">
                                            ...
                                            <div className="text-right">
                                                <span className="text-2xl font-bold font-heading text-brand-forest leading-none block">${pkg.price}</span>
                                                <span className="text-[8px] uppercase tracking-widest text-brand-charcoal/30 font-bold mt-1 block">Per Person</span>
                                            </div>
                                        </div>
                                    )}
                                </Link>
            ...
            <section className="bg-brand-charcoal text-brand-white py-16 border-y-2 border-brand-accent">
                <div className="container mx-auto px-6 md:px-12 grid grid-cols-2 md:grid-cols-4 gap-12">
                    {[
                        { icon: "✓", title: "Best Price", desc: "Find it cheaper? We'll match." },
                        { icon: "↺", title: "Flex Book", desc: "Cancel up to 48h before." },
                        { icon: "🛡", title: "Protected", desc: "Fully insured bookings." },
                        { icon: "✈", title: "Expert Care", desc: "Crafted by specialists." }
                    ].map((item, i) => (
                        <div key={i} className="text-center group border-r border-white/5 last:border-0">
                            <span className="text-2xl text-brand-accent mb-4 block group-hover:scale-110 transition-transform">{item.icon}</span>
            ...
            <section className="py-32 container mx-auto px-6 md:px-12">
                <div className="border-[3px] border-dashed border-brand-forest/20 p-12 md:p-24 flex flex-col lg:flex-row items-center gap-16">
                    ...
                    <div className="w-full lg:w-96 aspect-video bg-brand-charcoal/5 overflow-hidden group">
                        <img src="https://images.unsplash.com/photo-1544644013-3536ce48ca72?q=80&w=800" alt="Curator" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 grayscale" />
                    </div>
                </div>
            </section>
                ...
                <section className="bg-brand-charcoal py-24 border-t border-brand-accent/30 overflow-hidden">
                    <div className="container mx-auto px-6 md:px-12">
                        <div className="flex justify-between items-end mb-16">
                            <div>
                                <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-brand-accent mb-4">Back to explore?</h2>
                                <h3 className="text-3xl font-heading font-bold text-white">Recently Viewed</h3>
                            </div>
                            <button
