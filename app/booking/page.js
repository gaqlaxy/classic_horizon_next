    ...
    return (
      <div className="pt-40 pb-24 bg-brand-white min-h-screen flex items-center justify-center px-6">
        <div className="success-content bg-white p-12 md:p-20 shadow-2xl border border-brand-charcoal/5 rounded-sm text-center max-w-2xl w-full">
          ...
          <h1 className="text-4xl md:text-5xl font-heading font-bold text-brand-forest mb-6">
            Inquiry Received.
          </h1>
          <p className="text-brand-charcoal/60 text-lg mb-12">
            Thank you for choosing Classic Horizon, {formData.name}. One of our
            senior travel curators will contact you within 24 hours to craft
            your bespoke itinerary.
          </p>
          <Link
            href="/"
            className="inline-block px-12 py-5 bg-brand-forest text-brand-white font-bold uppercase tracking-widest text-xs hover:bg-brand-charcoal transition-all"
          >
            Return Home
          </Link>
        </div>
      ...
      <div className="container mx-auto px-6 max-w-3xl">
        <div className="flex items-center justify-between mb-20 relative">
          ...
          {[1, 2, 3].map((s) => (
            <div
              key={s}
              className={`w-10 h-10 flex items-center justify-center text-xs font-bold rounded-full border transition-all duration-500 ${
                step === s
        ...
        <form
          onSubmit={handleNext}
          className="bg-white p-8 md:p-16 shadow-xl border border-brand-charcoal/5"
        >
          {step === 1 && (
            <div className="form-step space-y-8">
              <h2 className="text-3xl font-heading font-bold text-brand-forest mb-10">
                Your Details
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full border-b-2 border-brand-charcoal/10 py-4 focus:outline-none focus:border-brand-forest transition-colors font-medium"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-brand-charcoal/40">
            ...
            <div className="form-step space-y-8">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="e.g., Santorini, Bali, Paris"
                  className="w-full border-b-2 border-brand-charcoal/10 py-4 focus:outline-none focus:border-brand-forest transition-colors font-medium"
                />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full border-b-2 border-brand-charcoal/10 py-4 focus:outline-none focus:border-brand-forest transition-colors font-medium bg-transparent"
                  >
          ...
          {step === 3 && (
            <div className="form-step space-y-8">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-brand-charcoal/40">
                  Reason for travel / Special Requests
                </label>
                <textarea
                  rows="4"
                  ...
                  placeholder="Honeymoon celebration, spiritual journey, dietary requirements etc."
                  className="w-full border-b-2 border-brand-charcoal/10 py-4 focus:outline-none focus:border-brand-forest transition-colors font-medium resize-none"
                ></textarea>
              </div>
              <div className="bg-brand-charcoal/5 p-6 space-y-2">
                ...
                <p className="text-sm text-brand-charcoal leading-relaxed">
                  Seeking a <strong>{formData.guests}</strong> journey to{" "}
                  <strong>{formData.destination || "a new horizon"}</strong>.
                  {formData.travelDate && (
                    <span>
                      {" "}
                      Target departure:{" "}
          ...
          <div className="mt-16 flex justify-between items-center">
            {step > 1 ? (
              <button
                type="button"
                onClick={handleBack}
                className="text-xs font-bold uppercase tracking-widest text-brand-charcoal/40 hover:text-brand-forest transition-colors"
              >
                Back
              </button>
            ) : (
